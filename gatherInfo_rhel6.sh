...
service --status-all | grep running|sort >> $out_file_path
 
echo -e "\n\n==================================== TOTAL RUNNING SERVICE ===================================\n" >> $out_file_path
echo -e "\tTotal Running service:\t"`service --status-all | grep running|sort| wc -l` >> $out_file_path
 
echo -e "\n\n========================================= HTTPD VERSION ======================================\n" >> $out_file_path

validate=`rpm -qa | grep httpd`
if [ $? -eq 0 ]; then
...
validateJava=`java -version 2>&1 >/dev/null | grep 'java version'`
if [ $? -eq 0 ]
then
...
echo -e "\n\n========================================= CRONTAB ============================================\n" >> $out_file_path
for user in $(cut -f1 -d: /etc/passwd); do crontab -u $user -l; done &>> $out_file_path

echo -e "\n\n========================================= GRUB INFO ==========================================\n" >> $out_file_path
cat /etc/grub.conf | grep -v '#' >> $out_file_path

echo -e "\n\n========================================= ULIMITS ============================================\n" >> $out_file_path
cat /etc/security/limits.conf | grep -v '#' >> $out_file_path
